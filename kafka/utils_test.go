...
func TestMapEq_subsetSemantics(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			result:   map[string]*string{"k1": &a, "k2": &b, "extra": &c},
			expected: map[string]*string{"k1": &a, "k2": &b},
			wantErr:  false,
		},
		{
			name:     "missing key in result",
			result:   map[string]*string{"k1": &a},
			expected: map[string]*string{"k1": &a, "k2": &b},
			wantErr:  true,
		},
		{
			name:     "value mismatch",
			result:   map[string]*string{"k1": &one, "k2": &two},
			expected: map[string]*string{"k1": &one, "k2": &three},
			wantErr:  true,
		},
		{
			name:     "identical maps",
			result:   map[string]*string{"k1": &one, "k2": &two},
			expected: map[string]*string{"k1": &one, "k2": &two},
			wantErr:  false,
		},
		{
			name:     "both nil maps",
		...
		},
		{
			...
			result:   map[string]*string{"k": nil},
			expected: map[string]*string{"k": nil},
			wantErr:  false,
		},
	}
